...
;(async () => {
  ...
  - Strong skills in \`JQuery\`, \`SQL\`, \`PHP\`
  - Excellent with the \`Javascript\` for web development
  - Proficient with \`Git\`
  - Learning the \`Agile\` mindset
  - Knowledge of \`TailwindCSS\`, \`Antd\` and others libraries
  - Skill in \`Unit Testing\`
  - Experience in \`React.js\`, \`Vue.js\` and the plugins around them
  - Familiar with \`AWS\`
...
async function getQuotes(time) {
  return await axios.get(QUOTES_API).then((response) => {
    if (response.data.length === 0) {
      return {
        quote:
          'There is no result without struggle, there is no struggle without sacrifice.',
        author: 'Me',
      }
    }
